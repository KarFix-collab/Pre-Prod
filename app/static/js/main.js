...
const KarFixApp = {
    ...
    initializeBootstrapComponents() {
        if (!window.bootstrap) {
            console.warn('Bootstrap JS is not available; skipping tooltips/popovers.');
            return;
        }
    ...
    initializeCharts() {
        ...
        const isAnalytics = pathName.includes('/administrator/analytics') || pathName.includes('/administrator/reports');
    ...
    formatCurrency(amount) {
        if (window.APP_CONFIG && typeof window.APP_CONFIG.formatCurrencyFn === 'function') {
            return window.APP_CONFIG.formatCurrencyFn(amount);
        }

        const currencyCode = window.APP_CONFIG?.currencyCode || 'ZAR';
        ...
        try {
            ...
            return `${window.APP_CONFIG?.currencySymbol || 'R'}${Number(amount || 0).toFixed(2)}`;
        }
    },
    ...
    showToast(message, type = 'info') {
        ...
        if (!window.bootstrap || !window.bootstrap.Toast) {
            console.warn('Bootstrap toast unavailable, falling back to alert:', message);
            alert(message);
            toast.remove();
            return;
        }
...
window.handleLogout = async function handleLogout(event) {
    ...
    const confirmed = window.confirm('Sign out of KarFix?');
    if (!confirmed) return;
    ...
    try {
        ...
        if (window.neonAuth && typeof window.neonAuth.signOut === 'function') {
            try { await window.neonAuth.signOut(); } catch (error) { console.warn(error); }
        }

        window.location.href = payload.redirect || '/auth/login';
    } catch (error) {
