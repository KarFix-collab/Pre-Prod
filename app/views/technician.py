...
def current_jobs(page=1, per_page=10):
    ...
    try:
        if request.method == 'POST':
            selected_job_id = request.form.get('job_select', type=int)
            if not selected_job_id:
                flash('Please select a job before proceeding.', 'warning')
                return redirect(url_for('technician.current_jobs', page=page, per_page=per_page))
            return redirect(url_for('technician.modify_job', job_id=selected_job_id))
...
@technician_bp.route('/jobs/<int:job_id>/update-service', methods=['POST'])
@handle_database_errors
def update_job_service(job_id):
    """Update a work order service quantity."""
    redirect_response = require_technician_login()
    if redirect_response:
        return redirect_response
    ...
    try:
        ...
        logger.error(f"Failed to update service quantity: {e}")
        flash('Failed to update service, please try again later', 'error')
        return redirect(url_for('technician.modify_job', job_id=job_id))


@technician_bp.route('/jobs/<int:job_id>/remove-service', methods=['POST'])
@handle_database_errors
def remove_job_service(job_id):
    """Remove a work order service."""
    redirect_response = require_technician_login()
    if redirect_response:
        return redirect_response
    ...
    try:
        ...
        flash('Failed to remove service, please try again later', 'error')
        return redirect(url_for('technician.modify_job', job_id=job_id))


@technician_bp.route('/jobs/<int:job_id>/update-part', methods=['POST'])
@handle_database_errors
def update_job_part(job_id):
    """Update a work order part quantity."""
    redirect_response = require_technician_login()
    if redirect_response:
        return redirect_response
    ...
    try:
        ...
        logger.error(f"Failed to update part quantity: {e}")
        flash('Failed to update part, please try again later', 'error')
        return redirect(url_for('technician.modify_job', job_id=job_id))


@technician_bp.route('/jobs/<int:job_id>/remove-part', methods=['POST'])
@handle_database_errors
def remove_job_part(job_id):
    """Remove a work order part."""
    redirect_response = require_technician_login()
    if redirect_response:
        return redirect_response
    ...
    try:
        ...
        logger.error(f"Failed to remove part: {e}")
        flash('Failed to remove part, please try again later', 'error')
        return redirect(url_for('technician.modify_job', job_id=job_id))


@technician_bp.route('/jobs/<int:job_id>/status', methods=['POST'])
@handle_database_errors
def update_job_status(job_id):
    """Update a work order workflow status."""
    redirect_response = require_technician_login()
    if redirect_response:
        return redirect_response
    ...
    try:
        ...
        success, errors, _ = job_service.update_job_status(job_id, new_status, changed_by_user_id=changed_by_user_id, note=note)
        if success:
...
@technician_bp.route('/jobs/<int:job_id>/assign-technician', methods=['POST'])
@handle_database_errors
def assign_technician(job_id):
    ...
    try:
        ...
        changed_by_user_id = session.get('user_id')
        success, errors, _ = job_service.assign_technician_to_job(job_id, technician_id, changed_by_user_id=changed_by_user_id)
        if success:
        ...
        flash('Failed to update technician assignment', 'error')
        return redirect(url_for('technician.job_detail', job_id=job_id))
...
def add_job_note(job_id):
    """Append an internal note to a work order."""
    redirect_response = require_technician_login()
    if redirect_response:
        return redirect_response
    ...
    try:
        ...
        changed_by_user_id = session.get('user_id')
        success, errors, _ = job_service.add_internal_note_to_job(job_id, note, changed_by_user_id=changed_by_user_id)
        if success:
...
def new_job():
    ...
    try:
        ...
        available_technicians = job_service.get_available_technicians()
        ...
        return render_template('technician/new_job.html',
                             ...
                             selected_mileage=None,
                             selected_job_date=None)
...
def create_job():
    ...
    try:
        ...
        mileage_raw = sanitize_input(request.form.get('mileage', '')).strip()
        mileage = None
        if mileage_raw:
            if not mileage_raw.isdigit():
                flash('Please enter a valid mileage', 'error')
                customers = customer_service.get_all_customers()
                return render_template('technician/new_job.html',
                                     customers=customers,
                                     min_date=date.today().isoformat(),
        ...
        if not customer_id or not validate_positive_integer(customer_id):
            ...
            return render_template('technician/new_job.html',
                                 ...
                                 selected_customer_id=customer_id,
                                 selected_vehicle_id=vehicle_id,
                                 selected_mileage=mileage_raw if 'mileage_raw' in locals() else None,
                                 selected_job_date=job_date_str if 'job_date_str' in locals() else None)

        if vehicle_id and not validate_positive_integer(vehicle_id):
            flash('Please select a valid vehicle', 'error')
            customers = customer_service.get_all_customers()
            return render_template('technician/new_job.html',
                                 customers=customers,
                                 available_technicians=job_service.get_available_technicians(),
        ...
        status = sanitize_input(request.form.get('status', Job.STATUS_IN_PROGRESS)).strip() or Job.STATUS_IN_PROGRESS
        technician_id = request.form.get('technician_id', type=int)

        success, errors, job = job_service.create_job(customer_id, job_date, vehicle_id=vehicle_id, mileage=mileage, status=status, assigned_to=technician_id, changed_by_user_id=session.get('user_id'))
...
def dashboard():
    ...
    try:
        ...
        for job in recent_jobs:
            ...
            if job_date == today or (isinstance(job_date, str) and job_date.startswith(str(today))):
...
@technician_bp.route('/calendar')
@handle_database_errors
@log_function_call
def workshop_calendar():
    ...
    try:
        today = date.today()
        calendar = job_service.get_workshop_calendar(start_date=today, end_date=today + timedelta(days=14))
        return render_template('technician/calendar.html', calendar=calendar, current_date=today)
    except Exception as e:
        logger.error(f"Failed to load workshop calendar: {e}")
        flash('Failed to load workshop calendar', 'error')
        return render_template('technician/calendar.html', calendar={'jobs': [], 'grouped_jobs': {}, 'job_count': 0, 'technicians': []}, current_date=date.today())


@technician_bp.route('/api/calendar')
@handle_database_errors
def api_workshop_calendar():
    """API: Get workshop calendar data."""
    try:
        ...
        return jsonify(job_service.get_workshop_calendar(tenant_id=tenant_id, start_date=start_date, end_date=end_date))
    except Exception as e:
        logger.error(f"Failed to load workshop calendar API: {e}")
