...
class Customer(db.Model, BaseModelMixin, TenantScopedMixin):
    ...
    preferred_tenant_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('tenant.tenant_id'), nullable=True, index=True)

    # Relationships
    jobs: Mapped[List["Job"]] = relationship("Job", back_populates="customer_rel", lazy="dynamic")
    vehicles: Mapped[List["Vehicle"]] = relationship("Vehicle", back_populates="customer", lazy="dynamic", cascade="all, delete-orphan")
    tenant: Mapped[Optional["Tenant"]] = relationship("Tenant", foreign_keys=[tenant_id], backref="customers")
    preferred_tenant: Mapped[Optional["Tenant"]] = relationship("Tenant", foreign_keys=[preferred_tenant_id])
    ...
    def get_vehicles(self) -> List["Vehicle"]:
        ...
        return self.vehicles.order_by(Vehicle.is_primary.desc(), Vehicle.make, Vehicle.model, Vehicle.year.desc()).all()

    def get_vehicle_history(self, vehicle_id: int) -> List["Job"]:
        """Get repair history for a specific vehicle."""
        from app.models.job import Job
        return (
            self.jobs.filter(Job.vehicle_id == vehicle_id)
